...
if datahmade:sismember("SNIEBR:gog"..bot_id, chat_id) then
var = true
else 
var = false
end
...
datahmade:sadd("SNIEBR:gog"..bot_id,idg) 
...
openChat(msg.chat_id_,SNIEBR_info)
datahmade:srem("SNIEBR:gog"..bot_id, msg.chat_id_)
datahmade:del('SNIEBR:'..bot_id.."charge:"..msg.chat_id_)
function SNIEBR_info2(k1,k2)
...
openChat(msg.chat_id_,SNIEBR_info2) 
--
end
else
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/SNIEBRabas/files_SNIEBR/master/getfile.json")
if res == 200 then
...
local json_file, res = https.request("https://raw.githubusercontent.com/SniebrFiles/files_SNIEBR/master/files_SNIEBR/"..file)
if res == 200 then
...
if text == 'تحديث' then
dofile('SNIEBR.lua')  
send(msg.chat_id_, msg.id_, 1, '♱┇لعيون ماركوس تم التحديث', 1, 'md')
end
if text == ("تحديث السورس") then
send(msg.chat_id_, msg.id_, 1, '♱┇لعيون ماركوس تم التحديث', 1, 'md')
os.execute('rm -rf ./libs/utils.lua')
os.execute('rm -rf SNIEBR.lua')
os.execute('cd libs;wget https://raw.githubusercontent.com/SNIEBRabas/SNIEBR/master/libs/utils.lua')
os.execute('wget https://raw.githubusercontent.com/SNIEBRabas/SNIEBR/master/SNIEBR.lua')
dofile('SNIEBR.lua')  
return false
end
...
datahmade:srem("SNIEBR:gog"..bot_id,group[i]) 
...
local gps2 = datahmade:smembers("SNIEBR:gog"..bot_id) or 0
for i=1,#gps do
datahmade:sadd("SNIEBR:gog"..bot_id, gps[i])
...
local gps2 = datahmade:smembers("SNIEBR:gog"..bot_id) or 0
for i=1,#gps do
datahmade:del("SNIEBR:gog"..bot_id)
...
local gpss = datahmade:smembers("SNIEBR:good"..bot_id..os.date("%d")) or 0
...
openChat(msg.chat_id_,SNIEBR_info2) 
--
datahmade:sadd("SNIEBR:gog"..bot_id, msg.chat_id_)
datahmade:set( 'SNIEBR:'..bot_id.."enable:"..msg.chat_id_,true)
end end
getUser(msg.sender_user_id_, TSby)
end
...
local gps2 = datahmade:scard("SNIEBR:gog"..bot_id) or 0
local gps9 = datahmade:scard("SNIEBR:good"..bot_id..os.date("%d")) or 0
...
datahmade:del("SNIEBR:gog"..bot_id,txt[2])
...
katu =  {'سحور','سياره','درايفر','قنفه','ايفون','بزونه','مطبخ','كرستيانو','دجاجه','مدرسه','الوان','غرفه','ثلاجه','كهوه','سفينه','العراق','محطه','طياره','رادار','منزل','ماركوس','كهرباء','تفاحه','اخطبوط','سلمون','فرنسا','برتقاله','تفاح','مطرقه','بتيته','لهانه','شباك','باص','سمكه','ذباب','تلفاز','حاسوب','انترنيت','حسن','جسر'};
name = katu[math.random(#katu)]
datahmade:set('SNIEBR:'..bot_id..'klmo'..msg.chat_id_,name)
 name = string.gsub(name,'سحور','س ر و ح')
name = string.gsub(name,'سياره','ه ر س ي ا')
name = string.gsub(name,'درايفر','د ا ف ر ي ر ')
...
name = string.gsub(name,'ماركوس','م ر ا ك س و')
...
name = string.gsub(name,'حسن','ح ن س')
...
local hres = redis:get('SNIEBR:'..bot_id..'hres')
local text =[[
...
👋┇اهلا بك في سورس درايفر 

🦁┇DRAEVER Team

📜┇[قناه شروحات سورس](t.me/DRAEVER_TV)

📥┇[قناه اغاني السورس](t.me/AY_AA)

🔎┇ [طريقه التنصيب](t.me/DRAEVER_TV)
...
datahmade:srem("SNIEBR:gog"..bot_id, '-100'..data.channel_.id_)
...
datahmade:srem("SNIEBR:gog"..bot_id,v) 
...
datahmade:sadd("SNIEBR:gog"..bot_id,v) 
